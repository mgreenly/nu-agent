...
RSpec.describe Nu::Agent::Tools::FileMove do
  let(:tool) { described_class.new }
  let(:test_dir) { File.join(Dir.pwd, "tmp", "file_move_test") }
  ...
  describe "#name" do
    it "returns the tool name" do
      expect(tool.name).to eq("file_move")
    end
  end

  describe "#description" do
    it "returns a description" do
      expect(tool.description).to include("PREFERRED tool for moving or renaming files")
    end

    it "mentions it can move between directories or rename in place" do
      expect(tool.description).to include("move between directories or rename in place")
    end
  ...
  describe "#execute" do
    ...
    context "with path resolution" do
      ...
      it "handles relative paths for source" do
        relative_source = File.join("tmp", "file_move_test", "source.txt")
      ...
      it "handles relative paths for destination" do
        relative_dest = File.join("tmp", "file_move_test", "destination.txt")
    ...
    context "when moving files successfully" do
      before do
        File.write(source_file, "test content")
      end
      ...
      it "returns success message" do
        ...
        expect(result[:message]).to include("moved successfully")
      end
    ...
    context "when renaming in same directory" do
      before do
        File.write(source_file, "test content")
      end

      it "renames the file successfully" do
        new_name = File.join(test_dir, "renamed.txt")
        result = tool.execute(arguments: { source: source_file, destination: new_name })
    ...
    context "when moving to different directory" do
      before do
        File.write(source_file, "test content")
      end

      it "moves file to subdirectory" do
        subdir_dest = File.join(test_dir, "subdir", "moved.txt")
      ...
      it "creates parent directories if they don't exist" do
        nested_dest = File.join(test_dir, "level1", "level2", "level3", "moved.txt")
    ...
    context "with move errors" do
      before do
        File.write(source_file, "test content")
      end

      it "handles StandardError during move operation" do
        # Mock FileUtils.mv to raise an error
        allow(FileUtils).to receive(:mv).and_raise(StandardError.new("Permission denied"))

        result = tool.execute(arguments: { source: source_file, destination: dest_file })

        expect(result[:status]).to eq("error")
        expect(result[:error]).to include("Failed to move file")
