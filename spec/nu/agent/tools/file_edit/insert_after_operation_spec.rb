...
require "spec_helper"
require "nu/agent/tools/file_edit/insert_after_operation"
require "tempfile"

RSpec.describe Nu::Agent::Tools::FileEdit::InsertAfterOperation do
  ...
  describe "#execute" do
    ...
    context "when inserting after pattern" do
      it "inserts content after first occurrence" do
        File.write(file_path, "hello world")
        ops = { pattern: "hello", content: " there" }

        result = operation.execute(file_path, ops)

        expect(result[:status]).to eq("success")
        expect(result[:operation]).to eq("insert_after")
        expect(File.read(file_path)).to eq("hello there world")
      end
    end

    context "when pattern appears multiple times" do
      it "inserts only after first occurrence" do
        ...
        expect(result[:status]).to eq("success")
        expect(File.read(file_path)).to eq("fooX bar foo baz")
      end
    end
  end
end
