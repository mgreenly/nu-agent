...
require "spec_helper"
require "nu/agent/commands/subsystems/messages_command"

RSpec.describe Nu::Agent::Commands::Subsystems::MessagesCommand do
  ...
  describe "#initialize" do
    it "initializes with correct subsystem name and config key" do
      ...
      expect(command.instance_variable_get(:@config_key)).to eq("messages_verbosity")
    end
  end
  ...
  describe "#execute" do
    it "shows help text with verbosity levels" do
      expect(application).to receive(:output_lines) do |*lines|
        expect(lines.flatten.any? { |line| line.include?("Messages Subsystem") }).to be true
        expect(lines.flatten.any? { |line| line.include?("0 - No message tracking output") }).to be true
      end
