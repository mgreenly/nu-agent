...
require "spec_helper"
require "nu/agent/commands/subsystems/spellcheck_debug_command"

RSpec.describe Nu::Agent::Commands::Subsystems::SpellcheckDebugCommand do
  ...
  describe "#initialize" do
    it "initializes with correct subsystem name and config key" do
      expect(command.instance_variable_get(:@subsystem_name)).to eq("spellcheck-debug")
      expect(command.instance_variable_get(:@config_key)).to eq("spellcheck_verbosity")
    end
  end
  ...
  describe "#execute" do
    it "shows help text with verbosity levels" do
      expect(application).to receive(:output_lines) do |*lines|
        expect(lines.flatten.any? { |line| line.include?("Spellcheck Debug Subsystem") }).to be true
        expect(lines.flatten.any? { |line| line.include?("0 - No spell checker output") }).to be true
      end
